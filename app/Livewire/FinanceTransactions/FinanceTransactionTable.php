...
final class FinanceTransactionTable extends PowerGridComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.date'), 'transaction_date_formatted', 'transaction_date')
                ->sortable(),

            Column::make(__('messages.reference'), 'reference_display', 'code')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.custom_period'), 'date_period')
                ->hidden(),

            Column::make(__('messages.category'), 'category_name', 'finance_category_id')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.type'), 'type_badge')
                ->sortable(),

            Column::make(__('messages.amount'), 'amount_formatted', 'amount')
                ->sortable()
    ...
    public function actions(FinanceTransaction $row): array
    {
        $actions = [
            Button::add('view')
                ...
                ->tooltip(__('messages.view_transaction')),
        ];
        ...
        if (is_null($row->reference_type)) {
            $actions[] = Button::add('edit')
                ...
                ->tooltip(__('messages.edit_transaction'));
            ...
            $actions[] = Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    ...
                    'method' => 'delete',
                    'params' => ['rowId' => $row->id],
                    'title' => __('messages.delete_transaction_title'),
                    'description' => __('messages.delete_transaction_confirmation'),
                ])
                ->tooltip(__('messages.delete_transaction'));
        }
    ...
    public function header(): array
    {
        return [
            Button::add('print-selected')
                ->slot('🖨️ ' . __('messages.print_selected'))
    ...
    public function bulkPrint(): void
    {
        ...
        if (empty($checkboxValues)) {
            $this->dispatch('toast', message: __('messages.no_transactions_selected'), type: 'warning');
            return;
        }
    ...
    public function delete($rowId, FinanceTransactionService $service): void
    {
        ...
        if ($transaction) {
            try {
                ...
                $this->dispatch('toast', message: __('messages.transaction_deleted'), type: 'success');
            } catch (\Exception $e) {

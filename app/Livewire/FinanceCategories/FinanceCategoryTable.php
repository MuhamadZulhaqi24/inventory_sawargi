...
final class FinanceCategoryTable extends PowerGridComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.type'), 'type_badge', 'type')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.slug'), 'slug')
                ->sortable()
                ->searchable(),
    ...
    public function actions(FinanceCategory $row): array
    {
        $actions = [
            Button::add('view')
                ...
                ->tooltip(__('messages.view_category')),
            ...
            Button::add('edit')
                ...
                ->tooltip(__('messages.edit_finance_category')),
        ];
        ...
        if (!in_array($row->name, $systemCategories)) {
            $actions[] = Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    ...
                    'method' => 'delete',
                    'params' => ['rowId' => $row->id],
                    'title' => __('messages.delete_finance_category') . '?',
                    'description' => __('messages.delete_finance_category_confirmation', ['name' => $row->name]),
                ])
                ->tooltip(__('messages.delete_finance_category'));
        }
    ...
    public function delete($rowId, FinanceCategoryService $service): void
    {
        ...
        if ($category) {
            ...
            if (in_array($category->name, ['Product Sales', 'Product Purchases'])) {
                $this->dispatch('toast', message: __('messages.system_category_no_delete'), type: 'error');
                return;
            }

            try {
                $service->deleteCategory($category);
                $this->dispatch('toast', message: __('messages.finance_category_deleted'), type: 'success');
            } catch (\Exception $e) {

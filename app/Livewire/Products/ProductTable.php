...
final class ProductTable extends PowerGridComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.sku'), 'sku')
                ->searchable(),

            Column::make(__('messages.name'), 'name_formatted', 'name')
                ->sortable()
                ->searchable()
                ->visibleInExport(false),
            ...
            Column::make(__('messages.categories'), 'category_name', 'category_id')
                ->sortable()
                ->searchable()
                ->visibleInExport(false),

            Column::make(__('messages.categories'), 'category_slug', 'category_id')
                ->hidden()
                ->visibleInExport(true),

            Column::make(__('messages.unit'), 'unit_symbol', 'unit_id')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.purchase_price'), 'purchase_price_formatted', 'purchase_price')
                ->sortable()
                ->bodyAttribute('text-right'),

            Column::make(__('messages.selling_price'), 'selling_price_formatted', 'selling_price')
                ->sortable()
                ->bodyAttribute('text-right'),

            Column::make(__('messages.margin'), 'margin_formatted')
                ->bodyAttribute('text-right text-indigo-600')
                ->visibleInExport(false),

            Column::make(__('messages.qty'), 'quantity')
                ->sortable()
                ->bodyAttribute('text-center'),

            Column::make(__('messages.min_stock'), 'min_stock')
                ->sortable()
                ->bodyAttribute('text-center'),

            Column::make(__('messages.status'), 'is_active_label', 'is_active')
                ->sortable()
                ->headerAttribute('text-center')
                ->bodyAttribute('text-center')
                ->visibleInExport(false),

            Column::make(__('messages.status'), 'is_active_export', 'is_active')
                ->hidden()
                ->visibleInExport(true),
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::boolean('is_active_label', 'is_active')
                ->label(__('messages.active'), __('messages.inactive')),
        ];
    }
    ...
    public function actions(Product $row): array
    {
        ...
        return [
            Button::add('view')
                ...
                ->tooltip(__('messages.view_product')),
            ...
            Button::add('edit')
                ...
                ->tooltip(__('messages.edit_product')),
            ...
            Button::add('delete')
                ...
                ->tooltip(__('messages.delete_product')),
        ];
    }
    ...
    public function delete($rowId, ProductService $service): void
    {
        ...
        if ($product) {
            try {
                ...
                $this->dispatch('toast', message: __('messages.product_deleted'), type: 'success');
            } catch (\Exception $e) {

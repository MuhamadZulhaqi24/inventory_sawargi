...
final class SalesTable extends PowerGridComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.invoice'), 'invoice_number')
                ->searchable()
                ->sortable(),

            Column::make(__('messages.customer'), 'customer_name', 'customer_id')
                ->searchable()
                ->sortable(),

            Column::make(__('messages.created_by'), 'created_by_name', 'created_by')
                ->sortable(),

            Column::make(__('messages.date'), 'sale_date_formatted', 'sale_date')
                ->sortable(),
    ...
    public function actions(Sale $row): array
    {
        return [
            ...
            Button::add('print')
                ...
                ->tooltip(__('messages.print_invoice')),
    ...
    public function delete($rowId, SaleService $saleService): void
    {
        ...
        if ($sale) {
            try {
                ...
                $this->dispatch('toast', message: __('messages.sale_deleted'), type: 'success');
